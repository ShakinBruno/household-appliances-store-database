...
(
    INSERT INTO MANAGEMENT.COUNTRIES (COUNTRY_NAME, COUNTRY_CODE)
...
NEW_BRANDS AS
(
    INSERT INTO MANAGEMENT.BRANDS (BRAND_NAME, COUNTRY, FOUNDED_YEAR, WEBSITE)
    ...
    INNER JOIN  NEW_COUNTRIES   AS N_COUNT ON UPPER(BRANDS.COUNTRY) = UPPER(N_COUNT.COUNTRY_NAME)
    WHERE       NOT EXISTS  (
                                SELECT  1
...
NEW_MODELS AS
(
    INSERT INTO MANAGEMENT.MODELS (MODEL_NAME, BRAND, RELEASE_YEAR)
    ...
    INNER JOIN  NEW_BRANDS  AS N_BRAND ON UPPER(MODELS.BRAND_NAME) = UPPER(N_BRAND.BRAND_NAME)
    ON CONFLICT DO NOTHING
    RETURNING   MODEL_ID,
                MODEL_NAME
),
NEW_CATEGORIES AS
(
    INSERT INTO MANAGEMENT.CATEGORIES (CATEGORY_NAME, DESCRIPTION)
...
NEW_APPLIANCES AS
(
    INSERT INTO MANAGEMENT.APPLIANCES (PRODUCT_NAME, MODEL, CATEGORY, DESCRIPTION)
    ...
    INNER JOIN  NEW_MODELS      AS N_MODEL  ON UPPER(APPLIANCES.MODEL_NAME) = UPPER(N_MODEL.MODEL_NAME)
    INNER JOIN  NEW_CATEGORIES  AS N_CAT    ON UPPER(APPLIANCES.CATEGORY_NAME) = UPPER(N_CAT.CATEGORY_NAME)
    ON CONFLICT DO NOTHING
...
NEW_SUPPLIERS AS
(
    INSERT INTO MANAGEMENT.SUPPLIERS (SUPPLIER_NAME, PHONE_NUMBER, EMAIL)
    ...
    WHERE       NOT EXISTS  (
                                SELECT  1
                                FROM    MANAGEMENT.SUPPLIERS AS SUPP
                                WHERE   UPPER(SUPPLIERS.SUPPLIER_NAME) = UPPER(SUPP.SUPPLIER_NAME)
                            )
    RETURNING   SUPPLIER_ID,
                SUPPLIER_NAME
),
NEW_APPLIANCE_SUPPLIES AS
(
    INSERT INTO MANAGEMENT.APPLIANCE_SUPPLIES (APPLIANCE, SUPPLIER, PRICE, STOCK_QUANTITY)
    ...
    INNER JOIN  NEW_APPLIANCES  AS N_APPL ON UPPER(APPLIANCE_SUPPLIES.APPLIANCE_NAME) = UPPER(N_APPL.PRODUCT_NAME)
    INNER JOIN  NEW_SUPPLIERS   AS N_SUPP ON UPPER(APPLIANCE_SUPPLIES.SUPPLIER_NAME) = UPPER(N_SUPP.SUPPLIER_NAME)
    ON CONFLICT DO NOTHING
),
NEW_CUSTOMERS AS
(
    INSERT INTO MANAGEMENT.CUSTOMERS (FIRST_NAME, LAST_NAME, PHONE_NUMBER, EMAIL)
    ...
    WHERE       NOT EXISTS  (
                                SELECT  1
                                FROM    MANAGEMENT.CUSTOMERS AS CUST
...
NEW_EMPLOYEES AS
(
    INSERT INTO MANAGEMENT.EMPLOYEES (FIRST_NAME, LAST_NAME, PHONE_NUMBER, EMAIL, JOB_POSITION, HIRE_DATE)
    ...
    WHERE       NOT EXISTS  (
                                SELECT  1
                                FROM    MANAGEMENT.EMPLOYEES AS EMPL
...
NEW_APPLIANCE_ORDERS AS
(
    INSERT INTO MANAGEMENT.APPLIANCE_ORDERS (CUSTOMER, EMPLOYEE, ORDER_DATE, TOTAL_AMOUNT, ORDER_STATUS)
    ...
    INNER JOIN  NEW_CUSTOMERS   AS N_CUST ON UPPER(APPLIANCE_ORDERS.CUSTOMER_NAME) = UPPER(N_CUST.FIRST_NAME || ' ' || N_CUST.LAST_NAME)
    INNER JOIN  NEW_EMPLOYEES   AS N_EMPL ON UPPER(APPLIANCE_ORDERS.EMPLOYEE_NAME) = UPPER(N_EMPL.FIRST_NAME || ' ' || N_EMPL.LAST_NAME)
    WHERE       NOT EXISTS  (
                                SELECT  1
                                FROM    MANAGEMENT.APPLIANCE_ORDERS AS APP_ORD
...
INSERT INTO MANAGEMENT.ORDER_ITEMS (APPLIANCE_ORDER, APPLIANCE, QUANTITY)
...
INNER JOIN  NEW_APPLIANCE_ORDERS AS N_ORD ON ORDER_ITEMS.ORDER_DATE = N_ORD.ORDER_DATE
INNER JOIN  NEW_APPLIANCES       AS N_APPL ON UPPER(ORDER_ITEMS.APPLIANCE_NAME) = UPPER(N_APPL.PRODUCT_NAME)
ON CONFLICT DO NOTHING;